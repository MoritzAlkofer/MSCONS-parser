...
def parse_UNA_segment(segment):
    ...
    Returns:
        dict: Dictionary containing the parsed separators and their meanings
...
def parse_UNB_segment(segment, separators):
    """
    Parse UNB (Nutzdaten-Kopfsegment) segment according to EDIFACT specifications.
    
    Args:
        unb_segment (str): The UNB segment string
        separators (dict): Dictionary containing element_separator and component_separator
    ...
    """
    segment_tag, *elements = split_with_release(segment, separators, separators['element_separator'])
    ...
    if len(elements) >= 7:  # Check for all required elements
        # S001 Syntax
        syntax_parts = split_with_release(elements[0], separators, separators['component_separator'])
        ...
        # S002 Sender
        sender_parts = split_with_release(elements[1], separators, separators['component_separator'])
        result['sender']['id-0004'] = sender_parts[0]         # 0004 (mandatory)
        if len(sender_parts) >= 2:
            result['sender']['qualifier-0007'] = sender_parts[1]  # 0007 (required)
        
        # S003 Receiver
        receiver_parts = split_with_release(elements[2], separators, separators['component_separator'])
        ...
        # S004 DateTime
        datetime_parts = split_with_release(elements[3], separators, separators['component_separator'])
...
def parse_UNH_segment(segment, separators):
        ...
        Args:
            ...
            separators (dict): Dictionary containing separator characters used in the message
        ...
        """
        elements = split_with_release(segment, separators, separators['element_separator'])
        ...
        message_type = elements[2].split(separators['component_separator'])
        ...
        if len(elements) > 3:
            transmission = elements[3].split(separators['component_separator'])
...
def parse_BGM_segment(segment, separators):
    # parse segment
    segment_tag, *elements = split_with_release(segment, separators, separators['element_separator'])
    # ensure its a BGM
    if segment_tag != 'BGM':
...
def parse_DTM_segment(segment, separators):
        ...
        if len(elements) >= 1:
            datetime_components = split_with_release(datetime_element, separators, separators['component_separator'])
...
def parse_RFF_segment(segment, separators):
    """
    Parse RFF (Reference) segment according to EDIFACT specifications.
    
    Args:
        segment (str): The RFF segment string
        separators (dict): Dictionary containing separator characters
    ...
    # parse RFF elements
    components = split_with_release(elements[0], separators, separators['component_separator'])
    if len(components) >= 2:
...
def parse_PIA_segment(segment, separators):
    """
    ...
    if len(elements) >= 2:
        ...
        product_components = split_with_release(elements[1], separators, separators['component_separator'])
...
def parse_QTY_segment(segment, separators):
    """
    ...
    if len(elements) >= 1:
        quantity_components = split_with_release(elements[0], separators, separators['component_separator'])
...
def parse_NAD_segment(segment, separators):
    """
    ...
    if len(elements) > 1:
        party_id_components = elements[1].split(separators['component_separator'])
...
def parse_UNS_segment(segment, separators):
    """
...
def parse_LOC_segment(segment, separators):
    ...
    if len(elements) >= 2:
        location_id = elements[1].split(separators['component_separator'])
        if len(location_id) >= 1:
            result['location_code-3225'] = location_id[0]
            
    # Related location 1 (C519)
    if len(elements) >= 3:
        related_loc = elements[2].split(separators['component_separator'])
        if len(related_loc) >= 1:
            result['first_related_code-3223'] = related_loc[0]
            
    return result

def parse_LIN_segment(segment, separators):
...
def parse_STS_segment(segment, separators):
    ...
    if len(elements) >= 1:
        status_category = elements[0].split(separators['component_separator'])
        if len(status_category) >= 1:
            result['status_category_code-9015'] = status_category[0]
            
    # Status reason (C556)
    if len(elements) >= 3:
        status_reason = elements[2].split(separators['component_separator'])
        if len(status_reason) >= 1:
            result['status_reason_code-9013'] = status_reason[0]
            
    return result

def parse_UNT_segment(segment, separators):
...
def parse_UNZ_segment(segment, separators):

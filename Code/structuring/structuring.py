...
def structure_message(segments):
...
def run_test(message, structure_message):
    ...
    print(f'\nValidating message structure integrity')
    print('Comparing segment tag frequencies between original and structured message to ensure no data was lost during structuring')
    print('\nheader')
    test(header_segments, structured_message['header'])
    print('\nbody')
    test(body_segments, structured_message['body'])
    print('\nfooter')
    test(summary_segments, structured_message['footer'])

if __name__ == '__main__':
    with open('../Data/Parsed_messages/MSCONS_TL_9905048000007_9985046000001_20250109_CS0000000GAZ65.json', 'r') as file:
        segments = json.load(file)

    structured_message = structure_message(segments)
    
    run_test(segments, structured_message)